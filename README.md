...
| Area | Path | Role |
| --- | --- | --- |
| Go services | `cmd/`, `pkg/`, `internal/wire/`, `proto/` | CLI `server` with existing chat services plus `presence`, `notification`, and `call`; HTTP/gRPC/WebSocket; Kafka/Redis/Cassandra |
...
| Kubernetes app | `deployments/helm/nexuschat` | Helm chart deploys 10 stateless NexusChat services plus optional Coturn, Service, Ingress, HPA, PDB, ServiceMonitor, NetworkPolicy, and ServiceAccount |
| Lab profile | `deployments/helm/nexuschat/values-lab-k3s.yaml` | K3s lab 4GB RAM/50GB disk: 1 replica, Recreate rollout, no Consul, no ServiceMonitor, no NetworkPolicy |
...
| `presence` | `server presence`, `nexuschat-presence` | HTTP + gRPC + WebSocket | Device heartbeat, online/last-seen state and presence events |
| `notification` | `server notification`, `nexuschat-notification` | HTTP + gRPC + WebSocket | Notification inbox, unread counts, preferences and VAPID Web Push |
| `call` | `server call`, `nexuschat-call` | HTTP + gRPC + WebSocket | 1-1 WebRTC signaling, call state machine, TURN credentials and 30-day call metadata |
...
- Presence and notification sockets run alongside chat; the chat header includes unread inbox, Web Push setup and a P2P WebRTC call panel.
...
6. Local development and lab deployment should be close to production without requiring a large cluster.
   ...
   - Helm values separate default/staging-like settings from the constrained `values-lab-k3s.yaml` profile.
...
helm template nexuschat deployments/helm/nexuschat \
  ...
  --values deployments/helm/nexuschat/values-lab-k3s.yaml > /tmp/nexuschat-lab.yaml
```
